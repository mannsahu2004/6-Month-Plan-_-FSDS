...
def eda_analysis(file_path):
    ...
    # Generate Data Visualozation
    plot_paths = generate_visualization(df)
    
    return f"\n Data Loaded Suceesfully!\n\n Summary:\n{summary}\n\n{missing_values}\n\n AI Insights:\n{insights}",plot_paths

# AI-Powered Insights using Misteral-78 (Ollama)
def generate_ai_insights(df_summary):
    prompt = f"Analyze the dataset summary and provide insights:\n\n{df_summary}"
    response = ollama.chat(model="misteral", messages=[{"role": "user", "content": prompt}])
...
def generate_visualization(df):
    ...
    # Histograms for numeric colummns
    for col in df.select_dtypes(include=['number']).columns:
    ...
    if not numeric_df.empty:
        ...
        sns.heatmao(numeric_df.corr(), annot=True, cmap='coolwarm', fmt=".2f", Linewidths=0.5)
...
demo = gr.Interface(
    ...
    title="📊LLM-Powered Exploratory Data Analyysis (EDA)",
